...
export const middleLabelsRelations: Partial<Record<BodyLabel, MiddleLabel>> = { SK: "Shared Kernel", P: "Partnership", C: "Customer/Supplier", S: "Customer/Supplier" }
...
export function mapEdgeLabels(rawLink: RawLink): Link {
	...
	for (const bodyLabel of bodyLabels) {
		if (rawLink.source.type.includes(bodyLabel)
			...
			if (!middleText) middleText = middleLabelsRelations[bodyLabel]
		}
	}
	if (
		...
		if (!middleText) middleText = "Customer/Supplier"
	}
	for (const boxLabel of boxLabels) {
		if (rawLink.source.type.includes(boxLabel)) boxSource = boxLabel
		if (rawLink.target.type.includes(boxLabel)) boxTarget = boxLabel
	}

	for (const bodyLabel of bodyLabels) {
		if (Object.keys(middleLabelsRelations).includes(bodyLabel)) break

		if (rawLink.source.type.includes(bodyLabel)) {
			if (!bodySource) bodySource = bodyLabel
			else bodySource += (", " + bodyLabel)
		}

		if (rawLink.target.type.includes(bodyLabel)) {
			if (!bodyTarget) bodyTarget = bodyLabel
			else bodyTarget += (", " + bodyLabel)
		}
	}
