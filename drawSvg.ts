...
import { Link } from './contextMap.js'

export function buildGraph(svg: D3Svg, { nodes, links }:{ nodes:  Node[], links: Link[]}, configuration: Configuration) {
...
export class ContextMap {
	...
	create(svg: D3Svg, links: Link[], nodes: Node[]) {
		...
		const contextMapNodes =
			nodes.map(node => ContextMapNode.createContextMapNode(node, this.configuration))
		ContextMapNode.disposeNodesInThePlane(contextMapNodes, { width, height }, this.configuration.nodeMargin)

		const contextMapLinks =
			ContextMapLink.createContextMapLinksFromNodes(contextMapNodes, links, this.configuration)
...
export class ContextMapLink {
	...
	static createContextMapLinksFromNodes(nodes: ContextMapNode[], links: Link[], config: Configuration): ContextMapLink[] {
		...
		for (const link of links) {
			...
			if (sourceNode && targetNode)
				contextMapLinks.push(ContextMapLink.createContextMapLink(
					sourceNode,
					targetNode,
					link,
					config
	...
	private static calculateLabelSize(config: Configuration, boxText: string | undefined, bodyText: string | undefined, labelIntersection: Point) {
		...
		const targetLabelSize: LabelSize = {
			...
			labelPosition: { x: labelIntersection.x - targetWidth/2 , y: labelIntersection.y - boxHeight/2  },
	...
	private appendPath(svg: D3Svg) {
		...
		svg
			...
			.attr("d", `M${sourceLabelPos.x},${sourceLabelPos.y}A0,0 0 0,1 ${targetLabelPos.x},${targetLabelPos.y}`);
	}

	private appendMiddleLabel(svg: D3Svg) {
		const calculateMidPoint = ([x1, y1]: [number, number], [x2, y2]: [number, number]) => [(x1 + x2) / 2, (y1 + y2) / 2]

		const midPoint = calculateMidPoint([this.sourcePoint.x, this.sourcePoint.y], [this.targetPoint.x, this.targetPoint.y])

		const middleLabel = svg.append("g")
		middleLabel.append("text")
...
export class ContextMapNode {
	...
	static disposeNodesInThePlane(nodes: ContextMapNode[], boxSize: { width: number, height: number }, margin = { horizontal: 0, vertical: 0 }) {
		const center = { x: 0, y: 0 }
		const nodeNumber = nodes.length

		const proposedColumns = Math.ceil(Math.sqrt(nodeNumber))

		let perRowTotalWidth: number[] = []
		let perRowTotalHeight: number[] = []
		...
		while (counter < nodes.length) {
			...
			for (let column = 0; column < proposedColumns; column++) {
				...
				if (largerWidth < boxSize.width) {
					...
					if (node.height > maxRowHeight) maxRowHeight = node.height + margin.vertical * 2
					counter++
				}
			}
	...
	static createContextMapNode(
		...
		return new ContextMapNode(
			width, height,
			textWidth, textHeight,
			configuration.font, node.id,
			{x: textX, y: textY}
		)
	}

	calculateIntersection({ x: x2, y: y2 }: Point, { x: centerX, y: centerY }: Point = { x: this.position.x, y: this.position.y }, { rx: a, ry: b }: EllipseSize = { rx: this.rx, ry: this.ry }) {
